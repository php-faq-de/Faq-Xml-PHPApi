...
namespace FXP\Api\Dao
{
    ...
    class Faq
    {
        ...
        public function setAlias( $value )
        {
            ...
            if ( $this->category->globalFaqAliasExists( $value ) ) { return false; }
        ...
        public function save( $xmlFolder )
        {
            ...
            $folder = \rtrim( $xmlFolder, \DIRECTORY_SEPARATOR ) . \DIRECTORY_SEPARATOR .
                      $this->category->getAlias() . \DIRECTORY_SEPARATOR . $this->alias;
            if ( !\is_dir( $folder ) ) { \mkdir ( $folder, 0755, true ); }
            $file = $folder . \DIRECTORY_SEPARATOR . 'faq.xml';
            $writer->openUri( $file );
        ...
        /**
         * Writes all FAQ Data (excluding the Answer HTML) to the XMLWriter (elementname is 'Faq')
         * 
        ...
        /**
         * Creates a new FAQ for defined category.
         *
         * @param \FXP\Api\Dao\Category $category The category of this FAQ
         * @param string $xmlFolder The 'xml' folder, where the whole PHP-FAQ is stored.
         * @param string $alias The unique FAQ alias
         * @param string $author The name of the initial FAQ author.
         * @param int|strin|null $created Optional unix timestamp of the FAQ creation date.
         * @param string $keywords All Keywords of this FAQ.
         * @param string $question The FAQ Question string
         * @param string $answer The FAQ Answer. (HTML)
         * @param array $editors All Editors of the FAQ
         * @return \FXP\Api\Dao\Faq Returns the new FAQ, added to defined category, or (boolean)FALSE if
         *         $xmlFolder|$alias|$author|$question|$answer is empty of invalid. It also return FALSE
         *         if $alias is already in use by a other FAQ.
         */
        public static function Create(
            \FXP\Api\Dao\Category $category, $xmlFolder,
            $alias, $author, $created, $keywords, $question, $answer, array $editors = array() )
        {
            if ( \is_null( $category ) ) { return false; }
            if ( empty( $xmlFolder ) || !\is_dir( $xmlFolder ) ) { return false; }
            if ( empty( $alias ) || empty( $author ) || empty( $question ) || empty( $answer ) ) { return false; }
            if ( !\FXP\Api\Dao\Faq::IsValidAlias( $alias ) ) { return false; }
            if ( $category->globalFaqAliasExists( $alias ) ) { return false; }
            $result = new \FXP\Api\Dao\Faq($category, $alias, $author, $created, $keywords, $question, $editors);
            $result->setAnswer( $answer, $xmlFolder );
            $result->save( $xmlFolder );
